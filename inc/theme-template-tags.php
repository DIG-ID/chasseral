...
function chasseral_theme_mega_menu_link( $slug, $type ) {
	...
	if ( ! empty( $slug ) ) :
		...
		if ( ! empty( $root ) ) :
			...
			$link_close  = '</a>';
			$link_title  = '<h2 class="font-bold text-xl lg:text-3xl font-sans text-black uppercase my-5">' . $root->post_title . '</h2>';
...
function chasseral_theme_sub_pages( $slug ) {
	...
	if ( ! empty( $slug ) ) :
		...
		$title_after   = '</span>';
		$title_overlay = '<p class="sub-page-link--title-overlay font-bold text-xl lg:text-lg xl:text-3xl font-sans text-black uppercase m-0 absolute top-[14px] transition-all duration-300 ease-in-out -translate-x-full opacity-0">' . esc_html__( 'eintreten', 'chasseral' ) . '</p>';
...
function chasseral_theme_see_all_posts( $slug = false, $name ) {
	...
	if ( $link ) :
		?>
			<a class="btn__arrow-alle" href="<?php echo esc_url( $link ); ?>"><?php echo sprintf( __( 'Alle %s', 'chasseral' ), $name ); ?>
 ...
 * Get notification.
 */
function chasseral_theme_notification() {
...
add_action( 'notification', 'chasseral_theme_notification' );