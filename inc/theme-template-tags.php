...
function chasseral_theme_sub_pages( $slug ) {
	...
	if ( ! empty( $slug ) ) :
		...
		$title_after   = '</span>';
		$title_overlay = '<p class="sub-page-link--title-overlay font-bold text-xl sm:text-lg xl:text-3xl font-sans text-black uppercase m-0 absolute top-[18px] xl:top-[14px] transition-all duration-300 ease-in-out -translate-x-full opacity-0">' . esc_html__( 'eintreten', 'chasseral' ) . '</p>';
...
function chasseral_theme_notification() {
	if ( get_field( 'show_notification', 'services-status' ) ) :
		?>
		<div class="notification flex items-strech border border-solid border-black mb-10">
