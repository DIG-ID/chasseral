	...
	<div class="card-event-wrapper grid grid-cols-1 lg:grid-cols-2 border-b border-solid border-black">
		<div class="card-event-image lg:border-r border-solid border-black ">
			...
			if ( has_post_thumbnail() ) :
				...
				?><img class="w-full h-auto object-cover lg:max-h-[700px]" src="<?php echo esc_url( esc_url( $img_url ) ); ?>" alt="<?php echo esc_html( get_the_post_thumbnail_caption() ); ?>"><?php
			else :
		...
		<div class="card-event-content flex flex-col justify-center items-start px-8 py-8 lg:pl-16 lg:pr-0 lg:py-0 bg-bg-color">
			...
			if ( get_post_type( get_the_ID() ) === 'agenda' ) :
				...
				echo '<a href="' . esc_url( get_permalink() ) . '" class="btn-general btn-general--szl">' . esc_html__( 'Eintrenten', 'chasseral' ) . '</a>';
			elseif ( get_post_type( get_the_ID() ) === 'events' ) :
				...
				echo '<p class="text-sm font-normal leading-7 max-w-md mb-8 line-clamp-4">' . esc_html( get_the_excerpt() ) . '</p>';
				echo '<a href="' . esc_url( get_permalink() ) . '" class="btn-general btn-general--hotel">' . esc_html__( 'Eintrenten', 'chasseral' ) . '</a>';
			endif;
			?>
		</div>
	</div>
